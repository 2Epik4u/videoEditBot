...
def combiner(videos, output="video.mp4", SILENCE="./SILENCE.mp3", print_info=True):
    ...
    final = ffmpeg.concat(*preparedVids, n=len(filteredVids), v=1, a=1).output(output).global_args("-y").global_args(
        "-vsync", "2").global_args("-hide_banner").global_args("-loglevel", "error")
    final.run()
...
if __name__ == "__main__":
    ...
    combiner(argv[2:] if len(argv) > 2 else sorted(["videos/" + i for i in listdir("videos")]),
             argv[1] if len(argv) > 1 else "video.mp4")
